...
import FactForm from './FactForm.vue'
...
function onSubmit(data: RawProjectFact) {
  emit('submit', data)
}
...
<template>
  <v-dialog v-model="show" persistent width="400">
    <v-card>
      <v-card-title>
        <span class="text-h5">
          {{ $t(`administration.facts.${props.type}.title`) }}
        </span>
      </v-card-title>
      <v-card-text>
        <FactForm ref="form" :value="props.value" @submit="onSubmit" />
      </v-card-text>
      <v-card-actions>
        <v-spacer />
