...
const formatedValue = computed(() => {
  return { ...props.value, date: isoToStandardString(props.value.date) }
})
...
<template>
  <v-dialog v-model="show" persistent width="1024">
    <v-card>
      <v-card-title>
        <span class="text-h5">{{ $t(`projectStatus.information.${props.type}.editTitle`) }}</span>
      </v-card-title>
      <v-card-text>
        <ItemForm ref="form" :value="formatedValue" @submit="onSubmit" />
      </v-card-text>
      <v-card-actions>
        <v-spacer></v-spacer>
