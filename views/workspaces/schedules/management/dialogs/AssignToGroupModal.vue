...
const tableDefinition: Options = {
    ...
    ajaxURL: !props.latest ? apiUrls.schedulesAssignedToGroups.replace(/%workspaceId/, workspaceId).replace(/%selectedProject/, selectedProject.value) :
        apiUrls.latestSchedulesAssignedToGroups.replace(/%workspaceId/, workspaceId).replace(/%selectedProject/, selectedProject.value),
    ...tabulatorOptions,
...
const onRowSelected = (row: RowComponent) => {
    ...
    updatedSelectedGroups.value = updatedSelectedGroups.value.map((version: any) => {
        if (version.id === data.id)
            return { ...version, project_present: true }
        else return { ...version }
    })
}
...
const onRowDeselected = (row: RowComponent) => {
    ...
    updatedSelectedGroups.value = updatedSelectedGroups.value.map((version: any) => {
        if (version.id === data.id)
            return { ...version, project_present: false }
        else return { ...version }
    })
}
onMounted(() => {
